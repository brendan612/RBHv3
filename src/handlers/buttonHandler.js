...
async function handlePickSidesButton(interaction) {
    ...
    try {
        await playerDraftService.setSidesAndCaptains(interaction.member.id, idParts[2]);
    } catch (error) {
...
async function handleStartChampDraftButton(interaction) {
    ...
    if (draft.host_id !== interaction.member.id && !hasRequiredRoleOrHigher(interaction.member, "admin")) {
        return await interaction.reply({
...
async function handleRedWinButton(interaction) {
    ...
    const canUse = draft.host_id == interaction.member.id || hasRequiredRoleOrHigher(interaction.member, "moderator");
    ...
    const draftManager = client.managers.draftManagerFactory.getDraftManager(draft_id);
