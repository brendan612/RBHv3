const { AutocompleteFocusedOption, Interaction, ChannelType } = require("discord.js");
const { sequelize, User, Lobby, Game, Season, Champion, Draft, DraftRound, Match, MatchPlayer, Region } = require("../models");
...
async function gameAutocomplete(focusedValue, interaction) {
    ...
    const filteredGames = gameNames.filter((game) => game?.toLowerCase().startsWith(focusedValue?.toLowerCase()));
    await interaction.respond(filteredGames.map((game) => ({ name: game, value: game })));
}

async function lobbyAutocomplete(focusedValue, interaction, filterToUser = false) {
    ...
    const filteredLobbyDetails = lobbyDetails
        .filter((detail) => {
            ...
            return detail.label.toLowerCase().includes(focusedValueLower) || detail.value.startsWith(focusedValue);
        })
...
async function seasonAutocomplete(focusedValue, interaction) {
    ...
    const seasonDetails = seasons.map((season) => ({
        label: `${season.name} (${formatDateToMMDDYYYY(season.start_date)} - ${formatDateToMMDDYYYY(season.end_date)})`,
...
async function championAutocomplete(focusedValue, interaction, match_id = -1) {
    ...
    if (match_id !== -1) {
        ...
        const takenChampions = match.Draft.DraftRounds.map((draftRound) => draftRound.champion_id);
        ...
        for (const [key, value] of cache) {
            ...
            if ((includes || emptySearch) && takenChampions.some((tc) => tc == value)) {
                champions.push({ key, value });
            }
        }
    } else {
        champions = client.cache.findByQuery(strippedSearch, "autoCompleteChampionData").slice(0, 25);
    }
...
async function regionAutocomplete(focusedValue, interaction) {
    ...
    const filteredRegions = focusedValue.length > 0 ? regionIDs.filter((region) => region?.toLowerCase().startsWith(focusedValue?.toLowerCase())) : regionIDs;
    await interaction.respond(filteredRegions.map((region) => ({ name: region, value: region })));
}
