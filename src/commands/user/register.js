...
module.exports = {
    data: new SlashCommandBuilder().setName("register").setDescription("Register your discord account with the server."),
    /**
     *
    ...
    async execute(interaction) {
        ...
        if (user) {
            ...
            await UserService.createUser(interaction.member.id, interaction.member.joinedAt);

            return await interaction.reply({
                content: "You have been registered! You should now verify your account.\n" + `Go to <#${channels.verify}> and type \`\`/verify\`\` to get started.`,
                ephemeral: true,
            });
        }
    },
