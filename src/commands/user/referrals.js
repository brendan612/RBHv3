...
const { SlashCommandBuilder, Interaction, User, Referral } = require("./index.js");
...
module.exports = {
    data: new SlashCommandBuilder().setName("referrals").setDescription("Get a leaderboard of users with the most referrals."),
    /**
     *
     * @param {Interaction} interaction
     */
    async execute(interaction, page = 0, pageSize = 10, commandInteraction = null) {
        ...
        const topReferrers = await UserService.getTopReferrersWithPagination(page, pageSize);
        ...
        const embed = baseEmbed("Referral Leaderboard", "Users with the most referrals");

        let message = "";
        for (const referrer of topReferrers) {
            try {
                const member = await guild.members.fetch(referrer.user_id);
                message += `${member.displayName.padEnd(30)} ${referrer.referralCount}\n`;
            } catch {
                const user = await User.findByPk(referrer.user_id);
                message += `${(user.summoner_name ? user.summoner_name : "Unknown User").padEnd(30)} ${referrer.referralCount} \n`;
            }
        }
