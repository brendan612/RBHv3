...
module.exports = {
    data: new ContextMenuCommandBuilder().setName("Mute").setType(ApplicationCommandType.User),
    /**
     *
    ...
    async execute(interaction) {
        ...
        await targeted_user.mute(interaction.member.id, expiration_date, "Muted by context menu");

        await interaction.reply({
            content: `User <@${targeted_user.user_id}> has been muted until ${displayDateAsTimestamp(expiration_date, TimestampFormat.Default)}`,
            ephemeral: false,
        });
    },
};
