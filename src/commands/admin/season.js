const { SlashCommandBuilder, Interaction, User, Season, Lobby, gameAutocomplete, gameOption, handleGameOption, seasonAutocomplete, seasonOption } = require("./index.js");
...
const { generateSeasonEndEmbed } = require("../../dataManager/messages/seasonEndEmbed.js");
const { getVeteransForSeason } = require("../../dataManager/queries/stats/stats");
const { getLeaderboard } = require("../../dataManager/queries/stats/leaderboard");
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) => option.setName("season_name").setDescription("Name of the season. Will default to season number if not provided").setRequired(false))
                .addStringOption((option) => option.setName("start_date").setDescription("Start date of the season. Format: MM/DD/YYYY. Defaults to today").setRequired(false))
                .addStringOption((option) => option.setName("end_date").setDescription("End date of the season. Format: MM/DD/YYYY. Defaults to end of the month").setRequired(false)),
        )
        .addSubcommand((subcommand) => subcommand.setName("end").setDescription("End the current season for a game").addStringOption(gameOption()))
        .addSubcommand((subcommand) => subcommand.setName("view").setDescription("View a season for a game.").addStringOption(gameOption()).addStringOption(seasonOption()))
        .addSubcommand((subcommand) =>
            subcommand.setName("generate-end-rewards").setDescription("Generate end of season rewards for a game. And Hall of Fame.").addStringOption(gameOption()).addStringOption(seasonOption()),
        ),
    /**
     *
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getSubcommand() === "create") {
            ...
            const newSeason = await Season.createSeason(season_name, game.game_id, start_date, end_date);
            ...
            const season = season_id == "current" ? await Season.getCurrentSeason(game.game_id) : await Season.findByPk(season_id);
            ...
            const qc_veteran_role = client.guild.roles.cache.find((role) => role.id === misc_roles.qc_veteran);

            const qc_top_veteran_role = client.guild.roles.cache.find((role) => role.id === misc_roles.qc_top_veteran);

            const qc_top_3_role = client.guild.roles.cache.find((role) => role.id === misc_roles.qc_top_3);

            const qc_champion_role = client.guild.roles.cache.find((role) => role.id === misc_roles.qc_champion);

            try {
                const veterans = await getVeteransForSeason(game.game_id, season_id, "NA");

                const topVeteran = veterans[0];
                const otherVeterans = veterans.slice(1);
                const topVeteranUser = await client.guild.members.fetch(topVeteran.user_id);
                topVeteranUser.roles.add(qc_top_veteran_role);
                const topVeteranService = await UserService.createUserService(topVeteran.user_id);
                await topVeteranService.addMoney(10000 * topVeteran.dataValues.total_matches); //top veteran
                ...
                otherVeterans.forEach(async (veteran) => {
                    ...
                    const userService = await UserService.createUserService(veteran.user_id);
                    await userService.addMoney(1000 * veteran.dataValues.total_matches); //veteran
                });

                const leaderboard = await getLeaderboard(game.game_id, season_id, "NA", 3, false);
                ...
                const userService = await UserService.createUserService(champion.user_id);
