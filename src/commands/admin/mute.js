    ...
    async execute(interaction) {
        ...
        if (await targeted_user.isMuted()) {
            ...
            await interaction.reply({
                content: `<@${targeted_user.user_id}> is already muted. \r\nExpires ${displayDateAsTimestamp(remaining, TimestampFormat.Relative)}`,
                ephemeral: true,
            });
            return;
        }
        await targeted_user.mute(interaction.member.id, expiration_date, interaction.options.getString("reason"));

        await interaction.reply({
            content: `User <@${targeted_user.user_id}> has been muted until ${displayDateAsTimestamp(expiration_date, TimestampFormat.Default)} for reason: ${interaction.options.getString("reason")}`,
            ephemeral: false,
        });
    },
};
