...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption((option) => option.setName("user").setDescription("User to level up").setRequired(true))
        .addIntegerOption((option) => option.setName("amount").setDescription("Amount of levels to level up").setRequired(true)),
    /**
     *
    ...
    async execute(interaction) {
        ...
        for (let i = 0; i < amount; i++) {
            const expToLevelUp = userLevelManager.expForNextLevel(userService.user.server_level);

            userService.user.server_experience += expToLevelUp + 1;
            userService.user.server_level++;

            await userLevelManager.assignLevelRole(user_id, userService.user.server_level);
        }
