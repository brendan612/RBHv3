const { SlashCommandBuilder, Interaction, User, Lobby, Draft, DraftRound, Champion, Sequelize, PlayerDraftRound } = require("./index.js");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        await playerDraftService.addPlayerToTeamByUserID(draft.red_captain_id, "red", 1);
        await playerDraftService.addPlayerToTeamByUserID(draft.blue_captain_id, "blue", 1);

        dummies = dummies.filter((dummy) => {
            return dummy.user_id !== draft.red_captain_id && dummy.user_id !== draft.blue_captain_id;
        });
        await playerDraftService.addPlayerToTeamByUserID(dummies[0].user_id, "blue", 2);
        await playerDraftService.addPlayerToTeamByUserID(dummies[1].user_id, "red", 3);
        await playerDraftService.addPlayerToTeamByUserID(dummies[2].user_id, "red", 3);
        await playerDraftService.addPlayerToTeamByUserID(dummies[3].user_id, "blue", 4);
        await playerDraftService.addPlayerToTeamByUserID(dummies[4].user_id, "blue", 4);
        await playerDraftService.addPlayerToTeamByUserID(dummies[5].user_id, "red", 5);
        await playerDraftService.addPlayerToTeamByUserID(dummies[6].user_id, "red", 5);
        await playerDraftService.addPlayerToTeamByUserID(dummies[7].user_id, "blue", 6);
        ...
        for (const round of draftManager.roundSequence) {
            if (round.action === "ban") {
                await championDraftService.addBan(round.team, champions[round.round - 1].champion_id, round.round);
            } else {
                await championDraftService.addPick(round.team, champions[round.round - 1].champion_id, round.round);
            }
        }
