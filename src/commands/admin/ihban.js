...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName("length").setDescription("Length of ban. EX: 1d, 1w, 1m, 1y, 1d1h1m1s, etc.").setRequired(true))
    ...
    async execute(interaction) {
        ...
        const userService = await UserService.createUserService(interaction.options.getUser("target").id);
        const { isBanned, remaining, reason } = await userService.isIHBanned();

        if (isBanned) {
            await interaction.reply({
                content: `<@${userService.user_id}> is already banned from inhouses\r\nExpires ${displayDateAsTimestamp(remaining, TimestampFormat.Relative)}\nReason: ${reason}`,
                ephemeral: false,
            });
            return;
        }

        await userService.ihban(interaction.user.id, expiration_date, interaction.options.getString("reason"));

        await interaction.reply({
            content: `User <@${userService.user_id}> has been banned from inhouses until ${displayDateAsTimestamp(
                expiration_date,
