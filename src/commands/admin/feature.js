    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) => option.setName("name").setDescription("Name of the feature").setRequired(true))
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) => option.setName("feature").setDescription("Name of the feature").setRequired(true).setAutocomplete(true))
                .addBooleanOption((option) => option.setName("enabled").setDescription("Enable or disable the feature").setRequired(true)),
        ),
    /**
     *
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "create-toggle") {
            ...
            await interaction.reply({
                content: `Feature \`\`${feature.name}\`\` has been created and is now ${enabled ? "enabled." : "disabled."}`,
                ephemeral: true,
            });
        } else {
            ...
            await interaction.reply({
                content: `Feature \`\`${feature.name}\`\` is now ${enabled ? "enabled." : "disabled."}`,
                ephemeral: true,
            });
        }
    },
