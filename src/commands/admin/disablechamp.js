...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addBooleanOption((option) => option.setName("disable").setDescription("Disable the champion").setRequired(true))
    ...
    async execute(interaction) {
        ...
        await ModerationLog.createModerationLog(interaction.user.id, interaction.user.id, expiration_date, "TOGGLECHAMP", champion.champion_id);

        await interaction.reply({
            content: `${champion.name} is now ${champion.enabled ? "enabled" : "disabled until " + displayDateAsTimestamp(expiration_date, TimestampFormat.ShortDate)}`,
        });
    },
