...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) => option.setName("name").setDescription("Name of the game").setRequired(true)),
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("toggle")
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "create") {
            ...
            await interaction.reply({
                content: `Game \`\`${game.name}\`\` has been toggled ${gamemode.enabled ? "on" : "off"}`,
            });
        }
    },
