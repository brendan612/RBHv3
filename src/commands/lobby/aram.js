...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addBooleanOption((option) => {
            return option.setName("random-teams").setDescription("Randomize teams").setRequired(false);
        }),
    /**
     *
    ...
    async execute(interaction) {
        ...
        if (randomTeams) {
            ...
            const availablePlayers = shuffledPlayers.filter((p) => {
                return p.user_id !== playerDraftManager.captains[0].user_id && p.user_id !== playerDraftManager.captains[1].user_id;
            });

            await playerDraftService.addPlayerToTeamByUserID(playerDraftManager.captains[0].user_id, "red", 1);
            await playerDraftService.addPlayerToTeamByUserID(playerDraftManager.captains[1].user_id, "blue", 1);
            ...
            while (availablePlayers.length > 0) {
                ...
                for (const player of players) {
                    await playerDraftService.addPlayerToTeamByUserID(player.user_id, playerDraftManager.currentTeam, playerDraftManager.currentRound);
                }
                playerDraftManager.currentRound++;
                playerDraftManager.currentTeam = playerDraftManager.currentTeam === "red" ? "blue" : "red";
            }
        }
        ...
        for (let i = 0; i < draftManager.roundSequence.length; i++) {
            ...
            while (picked_champs.includes(champion) || banned_champs.includes(champion)) {
                champion = champions[Math.floor(Math.random() * champions.length)];
            }

            if (action === "pick") {
                await championDraftService.handleChampSelect(interaction, champion, "pick", true);
                picked_champs.push(champion);
            } else {
                await championDraftService.handleChampSelect(interaction, champion, "ban", true);
