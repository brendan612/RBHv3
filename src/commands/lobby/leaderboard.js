...
const { countScriptCharacters } = require("../../utilities/utility-functions.js");

const { getLeaderboard } = require("../../dataManager/queries/stats/leaderboard.js");

const { generateLeaderboardImage } = require("../../dataManager/messages/leaderboardImage.js");

module.exports = {
    data: new SlashCommandBuilder().setName("leaderboard").setDescription("View the leaderboard for a game").addStringOption(gameOption()).addStringOption(seasonOption()),
    /**
     *
    ...
    async execute(interaction) {
        try {
            ...
            const message = await updateLeaderboard(interaction, page, game.game_id, season?.season_id, user.region_id);

            await collectButtonInteractions(interaction, message, game.game_id, season?.season_id, user.region_id);
        } catch (e) {
            console.log(e);
        }
    },
...
function createComponents(page, totalPages) {
    ...
    if (page <= totalPages && page > 2) {
        const button = new ButtonBuilder().setStyle(ButtonStyle.Secondary).setLabel("<<").setCustomId(`leaderboard_${1}`);
    ...
    if (page <= totalPages - 2) {
        const button = new ButtonBuilder().setStyle(ButtonStyle.Secondary).setLabel(">>").setCustomId(`leaderboard_${totalPages}`);
        components.addComponents(button);
    }
...
async function updateLeaderboard(interaction, page, game_id, season_id, region_id) {
    let game = null;
    let season = null;
    ...
    const image = await generateLeaderboardImage(offset, limit, game, season, region.region_id);

    const leaderboard = await getLeaderboard(game.game_id, season?.season_id, region.region_id, 3, false, true);
...
async function collectButtonInteractions(interaction, discordMessage, game_id, season_id, region) {
    const filter = (i) => {
        i.deferUpdate();
        return i.user.id === interaction.user.id && i.customId.startsWith("leaderboard");
    };
