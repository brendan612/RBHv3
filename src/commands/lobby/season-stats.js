...
const { getMatchesPlayedForSeason, getChampionMatchDataForServer, getCaptainStatsForServer } = require("../../dataManager/queries/stats/stats.js");
const { LeagueRoleEmojis } = require("../../assets/emojis.js");

module.exports = {
    data: new SlashCommandBuilder().setName("season-stats").setDescription("Server stats for a season").addStringOption(gameOption()).addStringOption(seasonOption()),
    /**
     *
    ...
    async execute(interaction) {
        ...
        const matches = await getMatchesPlayedForSeason(game.game_id, season?.season_id, user.region_id);
        ...
        const championStats = await getChampionMatchDataForServer(game.game_id, season?.season_id, user.region_id);
        ...
        const captainStats = await getCaptainStatsForServer(game.game_id, season?.season_id, user.region_id);
        ...
        embed.addFields(
            {
                ...
                value: top5Champions.map((c) => `${c.name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
            {
                name: "Least Played Champions",
                value: bottom5Champions.map((c) => `${c.name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
        );
        embed.addFields(
        ...
        embed.addFields(
            {
                ...
                value: highestWinRateChampions.map((c) => `${c.name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
            {
                name: "Lowest Win Rate Champions",
                value: lowestWinRateChampions.map((c) => `${c.name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
        );
        ...
        embed.addFields(
            {
                ...
                value: highestWinRateCaptains.map((c) => `${c.summoner_name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
            {
                name: "Lowest Captain WR",
                value: lowestWinRateCaptains.map((c) => `${c.summoner_name} | ${c.wins + c.losses} games | ${((c.wins / (c.wins + c.losses)) * 100).toFixed(2)}% WR`).join("\n"),
                inline: true,
            },
        );
