...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription("View information about a player in a match or update their information")
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "view") {
            ...
            const matchPlayer = match.MatchPlayers.find((mp) => mp.user_id === player.id);
            if (!matchPlayer) {
                return await interaction.reply({
            ...
            const matchPlayer = match.MatchPlayers.find((mp) => mp.user_id === user.id);
            ...
            const champion = await handleChampionOption(interaction, lobby.draft_id, true);
            ...
            return await interaction.reply({
                content: `Player: <@${user.id}>\nTeam: ${matchPlayer.team}\nRole: ${matchPlayer.role}\nChampion: ${champion.name}\n\nIf you are found to be deliberately inputting incorrect data, you will lose the privilege to use it and may face punishment.`,
                ephemeral: true,
            });
        }
    },
