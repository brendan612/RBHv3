const { SlashCommandBuilder, Interaction, gameAutocomplete, gameOption, handleGameOption, seasonOption, seasonAutocomplete, handleSeasonOption, Lobby, User, Game, baseEmbed } = require("./index.js");

const { Op } = require("sequelize");
const LobbyService = require("../../dataManager/services/lobbyService.js");
const LobbyDTO = require("../../dataManager/DTOs/lobbyDTO.js");
const { generateLobbyEmbed } = require("../../dataManager/messages/lobbyEmbed.js");

module.exports = {
    data: new SlashCommandBuilder().setName("host-stats").setDescription("Host Stats").addStringOption(gameOption()).addStringOption(seasonOption()),
    /**
     *
    ...
    async execute(interaction) {
        ...
        const embed = baseEmbed("Host Stats", `${season ? season.name : "All Seasons"}`);

        const sortedHostStats = new Map([...hostStats.entries()].sort((a, b) => b[1] - a[1]));
