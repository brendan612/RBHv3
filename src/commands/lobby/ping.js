...
module.exports = {
    data: new SlashCommandBuilder().setName("ping").setDescription("Ping the players in a lobby").addStringOption(gameOption()).addIntegerOption(lobbyOption()),
    /**
     *
    ...
    async execute(interaction) {
        ...
        const userString = lobbyDTO.players.map((user) => `<@${user.user_id}>`).join("\n");
