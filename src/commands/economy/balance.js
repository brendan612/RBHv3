...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) => subcommand.setName("check").setDescription("Check your balance or someone else's").addUserOption(userOption()))
        .addSubcommand((subcommand) =>
            subcommand
                .setName("give")
                .setDescription("Give money to another user")
                .addUserOption(userOption("target", "Targeted User", true))
                .addStringOption((option) => option.setName("amount").setDescription("Amount of money to give").setRequired(true)),
        )
        .addSubcommand((subcommand) => subcommand.setName("top").setDescription("View the users with the most money on the server")),
    /**
     *
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "check") {
            ...
            if (target) {
                ...
                const formatted_server_money = new Intl.NumberFormat("en-US", {}).format(userToDisplay.server_money);
            ...
            const embed = baseEmbed("Balance", `You have given <@${target.id}> ${amount} :pound:`, false);

            return interaction.reply({
                embeds: [embed],
            });
        } else if (interaction.options.getSubcommand() === "top") {
            const embed = baseEmbed(":pound: LEADERBOARD :pound:", "Users with the most money.");
            ...
            for (const user of users) {
                const formatted_server_money = new Intl.NumberFormat("en-US", {}).format(user.server_money);
