...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) => {
			return option
				.setName("suit")
				.setDescription("The suit you want to bet on.")
				.setRequired(true)
				.setChoices(
					{
						name: options[0][0],
	...
	async execute(
		...
		const result = won
			? `You won ${winAmount} :pound:`
			: `You lost ${amount} :pound:`;

		const embed = baseEmbed(
			`The correct suit was ${selectedOption}`,
			result,
			false,
			"Suits Game"
		);
