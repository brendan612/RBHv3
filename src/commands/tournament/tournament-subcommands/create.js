...
const generateTournamentCreateModal = async (interaction) => {
	const formModal = new ModalBuilder()
		.setCustomId("tournament_create_form")
	...
	const description = new TextInputBuilder()
		...
		.setPlaceholder("Enter the tournament description")
...
const handleTournamentCreate = async (interaction) => {
	...
	TournamentManager.startTournamentCreation(
		interaction.user.id,
		name,
		description
	);

	const button = createNextStepButton(
		"Next Step: Tournament Dates",
		"tournament_create_form_dates_button"
	);
...
const handleFollowUpTournamentDatesButton = async (interaction) => {
	const current_tournament = TournamentManager.getTemporaryTournament(
		interaction.user.id
	);
	...
	const formModal = new ModalBuilder()
		.setCustomId("tournament_create_form_dates")
		.setTitle("Create a new Tournament");
	...
	const registrationStartActionRow = new ActionRowBuilder().addComponents(
		registration_start_date
	);
	formModal.addComponents(registrationStartActionRow);

	const registrationEndActionRow = new ActionRowBuilder().addComponents(
...
const handleTournamentDatesSubmit = async (interaction) => {
	...
	let registration_start_date = new Date(
		interaction.fields.getTextInputValue("registration_start_date")
	);
	let registration_end_date = new Date(
		interaction.fields.getTextInputValue("registration_end_date")
	);
	...
	const allValidDates =
		isValidDate(registration_start_date) &&
		isValidDate(registration_end_date) &&
	...
	TournamentManager.addTournamentDates(
		interaction.user.id,
		registration_start_date,
		registration_end_date,
		start_date,
	...
	const tournament = TournamentManager.getTemporaryTournament(
		interaction.user.id
	);
	...
	await interaction.message.edit({
		...
		components: [
			createNextStepButton(
				"Next Step: Select Game and Formats",
				"tournament_create_form_game_button"
			),
		],
	});
};
...
const handleTournamentGameFormatButton = async (interaction) => {
	...
	const gameOptions = games.map((game) => {
		return new StringSelectMenuOptionBuilder()
			.setLabel(game.name)
			.setValue(game.game_mode_id.toString());
	});

	const select = new StringSelectMenuBuilder()
		.setCustomId("tournament_game_format_selection")
...
const handleTournamentGameFormatSelectMenu = async (interaction, values) => {
	...
	TournamentManager.addGameFormat(
		interaction.user.id,
		gameMode.game_id,
	...
	const registration_type_select = new StringSelectMenuBuilder()
		...
		.addOptions(
			...
			new StringSelectMenuOptionBuilder()
				...
				.setValue("INVITE"),
			new StringSelectMenuOptionBuilder()
				.setLabel("Leaderboard Registration")
				.setValue("LEADERBOARD")
		)
		.setMaxValues(1)
		.setMinValues(1);

	const registrationComponents = new ActionRowBuilder().addComponents(
		registration_type_select
	);

	await interaction.message.edit({
		content:
			"Tournament game mode has been set.\n```" +
			JSON.stringify(tournament, null, 2) +
			"```",
...
const handleTournamentRegistrationTypeSelectMenu = async (
	interaction,
	values
) => {
	await interaction.deferReply();
	...
	const elimination_type_select = new StringSelectMenuBuilder()
		...
		.addOptions(
			new StringSelectMenuOptionBuilder()
				...
				.setValue("SINGLE_ELIMINATION"),
			new StringSelectMenuOptionBuilder()
				.setLabel("Double Elimination")
				.setValue("DOUBLE_ELIMINATION"),
			new StringSelectMenuOptionBuilder()
	...
	const eliminationComponents = new ActionRowBuilder().addComponents(
		elimination_type_select
	);

	await interaction.message.edit({
		content:
			"Tournament registration type has been set.\n```" +
			JSON.stringify(tournament, null, 2) +
			"```",
...
const handleTournamentEliminationTypeSelectMenu = async (
	interaction,
	values
) => {
	await interaction.deferReply();
	...
	await interaction.message.edit({
		content:
			"Tournament elimination type has been set.\n```" +
			JSON.stringify(tournament, null, 2) +
			"```",
		components: [],
	});
