...
const { ButtonBuilder, ButtonStyle, Interaction, ActionRowBuilder, GuildMember, Client } = require("discord.js");
const { getSummonerByName, getRankBySummonerId } = require("../api/riot/riotApiHandler.js");
    ...
    class User extends Model {
        ...
        awardVoteMoney = async (vote, client) => {
            ...
            if (vote.type !== "upvote") {
                ...
                const time_since_last_vote = moment.unix(current_time).diff(lastVoteDate, "days");
            ...
            const embed = baseEmbed("Thanks for voting for RBH!", `<@${this.user_id}> has been awarded ${moneyToAward} :pound: for voting on top.gg!`, true, "Vote Rewards");

            if (this.vote_streak >= 2) {
                embed.addFields({
                    name: "Streak",
                    value: `You are on a ${Math.floor(this.vote_streak / 2)} day voting streak!`,
                    inline: false,
                });
            }

            const actionRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("Vote").setURL(`https://top.gg/servers/${vote.guild}/vote`).setStyle(ButtonStyle.Link));
        ...
        mute = async (moderator_id, duration, reason) => {
            ...
            const log = await ModerationLog.createModerationLog(moderator_id, this.user_id, duration, ActionType.MUTE, reason);

            const member = await client.guilds.cache.get(client.guildID).members.fetch(this.user_id);
        ...
        unmute = async (moderator_id, reason) => {
            const { ModerationLog } = require("../models");
            const log = await ModerationLog.createModerationLog(moderator_id, this.user_id, null, ActionType.UNMUTE, reason);
    ...
    User.init(
        {
            ...
            puuid: {
                type: DataTypes.STRING,
                allowNull: true,
                comment: "PUUID for league of legends - Populated on verify via Riot API",
            },
            server_experience: {
