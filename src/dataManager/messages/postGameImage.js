 ...
 * @returns
 */
async function generatePostGameImage(match) {
    const matchDTO = new MatchDTO(match);
...
async function drawBackground(ctx, width, height, winningTeam) {
    const backgroundImagePath = path.join(__dirname, `../../assets/postgame/${winningTeam}_win.png`);
...
async function drawMatchDetails(ctx, match) {
    ...
    ctx.fillStyle = "#fff";
    ctx.fillText(lobby.lobby_name, canvasWidth / 2 - lobbyNameMetrics.width / 2, 100);

    const matchDate = new Date(match.start_time);

    const matchDateText = `${matchDate.getMonth() + 1}/${matchDate.getDate()}/${matchDate.getFullYear()}`;
    ...
    ctx.fillText(hostName, canvasWidth - hostNameMetrics.width - 10, canvasHeight - 10);
}
async function drawPlayers(ctx, players, winningTeam) {
    ...
    ctx.font = "bold 70px Noto Serif KR";
    ...
    for (let i = 0; i < 5; i++) {
        ...
        ctx.strokeText(user.summoner_name + "#" + user.tag_line, blueTeamStartX, blueTeamStartY + i * blueTeamSpacing);

        ctx.fillStyle = "#104ee0";
        ctx.fillText(user.summoner_name + "#" + user.tag_line, blueTeamStartX, blueTeamStartY + i * blueTeamSpacing);

        ctx.strokeStyle = strokeStyle;
        ctx.lineWidth = lineWidth;
        ctx.strokeText(playerEloChange, blueTeamStartX + longestPlayerName + 100, blueTeamStartY + i * blueTeamSpacing);

        ctx.fillStyle = winningTeam === "blue" ? "#57de5b" : "#a10003";
        ctx.fillText(playerEloChange, blueTeamStartX + longestPlayerName + 100, blueTeamStartY + i * blueTeamSpacing);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        ctx.strokeStyle = strokeStyle;
        ctx.lineWidth = lineWidth;
        ctx.strokeText(user.summoner_name + "#" + user.tag_line, redTeamStartX, redTeamStartY + i * redTeamSpacing);

        ctx.fillStyle = "#e01010";
        ctx.fillText(user.summoner_name + "#" + user.tag_line, redTeamStartX, redTeamStartY + i * redTeamSpacing);

        ctx.strokeStyle = strokeStyle;
        ctx.lineWidth = lineWidth;
        ctx.strokeText(playerEloChange, redTeamStartX + longestPlayerName + 100, redTeamStartY + i * redTeamSpacing);

        ctx.fillStyle = winningTeam === "red" ? "#57de5b" : "#a10003";
        ctx.fillText(playerEloChange, redTeamStartX + longestPlayerName + 100, redTeamStartY + i * redTeamSpacing);
