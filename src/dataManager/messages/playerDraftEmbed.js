...
async function generatePlayerDraftEmbed(draft, sendMessage = true) {
    ...
    embed
        .setFooter({
            text: `Hosted by ${host.nickname ?? host.user.globalName} • Lobby ID: ${lobby.lobby_id} • Seasonal Lobby ID: ${lobby.season_lobby_id}`,
    ...
    if (playerDraftRounds.length === 0) {
        ...
        const lowestCaptain = captains.sort((a, b) => a.elo_rating - b.elo_rating)[0];
        ...
        const rows = [components, new ActionRowBuilder().addComponents([opggButton])];
        const message = await sendEmbedMessage(lobby, draft, embed, rows);
        return message;
    } else if (playerDraftRounds.length === 9) {
    } else {
        playerDraftManager.currentRound = playerDraftRounds.sort((a, b) => b.round_number - a.round_number)[0].round_number + 1;
    }
    ...
    for (const playerDraftRound of playerDraftRounds) {
        availablePlayers = availablePlayers.filter((player) => player.user_id !== playerDraftRound.user_id);
    ...
    if (availablePlayers.length > 0) {
        ...
        if (playerDraftManager.currentTeam === "blue") {
            embed.setDescription(`<@${playerDraftManager.blue_captain.user_id}> **is picking ${maxPicks} player${maxPicks > 1 ? "s" : ""}**`);
        } else {
            embed.setDescription(`<@${playerDraftManager.red_captain.user_id}> **is picking ${maxPicks} player${maxPicks > 1 ? "s" : ""}**`);
        }
    } else {
        embed.setDescription(`**All players have been drafted. Host can now begin Champ Draft**`);
    }
    ...
    if (availablePlayers.length > 0) {
        const components = await generatePlayerDraftComponents(availablePlayers, draft.draft_id);
...
async function generatePlayerDraftComponents(players, draft_id) {
    const playerDraftManager = client.managers.playerDraftManagerFactory.getPlayerDraftManager(draft_id);
    const ranks = await PlayerDraftManager.getRanks(players);
    ...
    for (const player of players) {
        select.addOptions(
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription(`${player.primary_role} | ${ranks.get(player.user_id).toUpperCase()}`)
...
function generateChampionDraftComponents(draft_id) {
    const start = new ButtonBuilder().setCustomId(`startchampdraft_${draft_id}`).setLabel("Start Champ Select").setStyle(ButtonStyle.Primary);
...
function generateSideSelectionButtons(draft) {
    const blueButton = new ButtonBuilder().setCustomId(`pick_${draft.draft_id}_blue`).setLabel("Blue").setStyle(ButtonStyle.Primary);

    const redButton = new ButtonBuilder().setCustomId(`pick_${draft.draft_id}_red`).setLabel("Red").setStyle(ButtonStyle.Danger);

    const actionRow = new ActionRowBuilder().addComponents([blueButton, redButton]);
