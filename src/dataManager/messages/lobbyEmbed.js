...
async function generateLobbyEmbed(lobby, sendMessage = true) {
    return new Promise(async (resolve, reject) => {
        ...
        const embed = baseEmbed(lobby.lobby_name, lobby.game_name + " In-House Lobby");
        embed
            .setFooter({
        ...
        if (sendMessage) {
            ...
            if (lobby.message_id) {
                try {
                    ...
                    const message = await lobby.channels.get("general").messages.fetch(lobby.message_id);
                    if (message) {
...
function generateLobbyButtons(lobby) {
    const joinButton = new ButtonBuilder().setStyle(ButtonStyle.Success).setLabel("Join").setCustomId(`join_${lobby.lobby_id}`);
    const dropButton = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel("Drop").setCustomId(`drop_${lobby.lobby_id}`);
    const draftButton = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel("Draft").setCustomId(`draft_${lobby.lobby_id}`);
