...
const veteransBackground = path.join(__dirname, "../../assets/images/leaderboard/leaderboard.jpg");
 ...
 * @returns
 */
async function generateLeaderboardImage(offset, limit, game, season, region = "NA") {
    const cachedLeaderboard = client.cache.get(`leaderboard_image-${game.game_id}-${season.season_id}-${region}}-${offset}-${limit}`, "leaderboard");

    if (cachedLeaderboard) {
        return cachedLeaderboard;
    }

    let leaderboard = await getLeaderboard(game.game_id, season?.season_id, region, 3, false, true);
    ...
    for (let i = 0; i < leaderboard.length; i++) {
        ...
        ctx.strokeText(winLoss, borderOffset + spacing + maxNameWidth + 225, playerYStart + i * spacing);
        ctx.fillText(winLoss, borderOffset + spacing + maxNameWidth + 225, playerYStart + i * spacing);
    }
    ...
    client.cache.set(`leaderboard_image-${game.game_id}-${season.season_id}-${region}-${offset}-${limit}`, attachment, "leaderboard");
