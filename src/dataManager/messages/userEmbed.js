...
async function generateVerifyEmbed(user_id, game_name, tag_line, referrer) {
    ...
    const embed = baseEmbed(`Verify ${game_name}#${tag_line}`, "Verify your account with your Riot ID");

    tag_line = tag_line.replace("#", "");

    embed.addFields({
        name: "Steps to Verify",
        value: "1. Change your summoner icon to the one shown to the right.\n2. Click the verify button below.",
        inline: false,
    });
    embed.addFields({
        name: `Account Requirements`,
        value: "\u2022 Account must be level 50.\n\u2022 Account must be at least Gold 4",
        inline: false,
    });

    let verifyIcon = 0;
    const summoner = await getSummonerByRiotID(game_name, tag_line, user.region_id);
    if (!summoner) {
        return null;
    }
    ...
    embed.setThumbnail(`http://ddragon.leagueoflegends.com/cdn/13.18.1/img/profileicon/${verifyIcon}.png`);
    ...
    const verifyButton = new ButtonBuilder().setCustomId(customId).setLabel("Verify").setStyle(ButtonStyle.Primary);
...
async function generateLevelUpEmbed(user_id, level, awardMoney, roleTitle) {
    ...
    const embed = baseEmbed(`Level Up!`, `<@${user_id}> has reached level ${level} - ${roleTitle}!`);
    if (awardMoney) {
        embed.addFields({
            name: "Level Up Reward",
            value: `You have been awarded ${parseInt(Math.floor(awardMoney))} :pound:`,
            inline: false,
        });
    }
    const channel = await guild.channels.fetch(channels.games["League of Legends"]);
...
async function generateProfileEmbed(interaction, user_id) {
    ...
    const { level, remainingExp } = userLevelManager.calculateLevelAndRemainingExp(user.server_experience);
    ...
    if (!donorRole && !isBooster && !staff) {
        const embed = await generateBasicEmbed(user, userLevelManager.expForNextLevel(level), remainingExp, stats.wins, stats.losses, stats.elo_rating);
        ...
        const background = await drawBackgroundImage(context, canvas, donorRole, isBooster, level, staff);
 ...
 */
async function drawBackgroundImage(ctx, canvas, donorRole, isBooster, level, staff = false) {
    if (!staff) {
        const { suitNumber, suitName, cardNumber } = userLevelManager.getSuitNumberNameAndCardNumber(level);

        const backgroundImagePath = path.join(__dirname, `../../assets/profiles/${suitName}.png`);
        const backgroundImage = await loadImage(backgroundImagePath);

        ctx.drawImage(backgroundImage, 0, 0, canvas.width, canvas.height);
    } else {
        const backgroundImagePath = path.join(__dirname, `../../assets/profiles/queen_of_hearts.png`);
        const backgroundImage = await loadImage(backgroundImagePath);
    ...
    if (banner) {
        const bannerImagePath = path.join(__dirname, `../../assets/profiles/${banner}.png`);
        const bannerImage = await loadImage(bannerImagePath);
        ctx.drawImage(bannerImage, 0, 0, canvas.width, canvas.height);
    }
}

async function generateBasicEmbed(user, exp, remainingExp, wins, losses, elo_rating) {
    const guild = await client.guilds.fetch(client.guildID);
    ...
    const embed = baseEmbed(`${user.summoner_name}#${user.tag_line}`, "Server Profile");
    ...
    embed.addFields(
        ...
        {
            name: "Season Record",
            value: `${wins}W - ${losses}L (${Math.round((wins / (wins + losses)) * 100, 2)}%) | ${elo_rating}`,
            inline: true,
        },
    );
