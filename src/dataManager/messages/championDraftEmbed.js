...
const { Lobby, User, PlayerDraftRound, Draft, DraftRound, Champion, Match } = require("../../models");
const { baseEmbed } = require("../../components/embed.js");

const { inhouse_icon_url, channels } = require(`../../../${process.env.CONFIG_FILE}`);
...
const { displayDateAsTimestamp, TimestampFormat } = require("../../utilities/timestamp.js");

const LobbyService = require("../services/lobbyService.js");
const DraftService = require("../services/draftService.js");
const { generateWinInputComponents } = require("../../components/embeds/winButtonComponents.js");
const { generateTeamPlayerList } = require("../../utilities/utility-functions.js");
...
const canvasHeight = headerBarHeight + banSectionHeight + namePlateHeight + pickSectionHeight;

const banPlaceholderPath = path.join(__dirname, "../../assets/drafting/ban_placeholder.png");

const pickPlaceholderPath = path.join(__dirname, "../../assets/drafting/pick_placeholder.png");
...
async function generateChampionDraftEmbed(draft, sendMessage = true) {
    ...
    embed
        .setFooter({
            text: `Hosted by ${host.nickname ?? host.user.globalName} • Lobby ID: ${lobby.lobby_id}`,
    ...
    if (roundInfo) {
        if (roundInfo.team === "blue") {
            ...
            ctx.fillRect(0, 0, pickSectionWidth * 5 + pickSectionWidth / 2, canvasHeight);
        } else {
            ctx.fillStyle = "#a10003";
            ctx.fillRect(pickSectionWidth * 6 - pickSectionWidth / 2, 0, pickSectionWidth * 5 + pickSectionWidth / 2, canvasHeight);
        }
    }

    await generateHeaderBar(ctx, red_captain.summoner_name, blue_captain.summoner_name);

    await generateBanSection(ctx, draftManager.blue_team_bans, draftManager.red_team_bans, lobby.lobby_id);

    //await generateNamePlateSection(ctx, blue_team, red_team);

    await generatePickSection(ctx, draftManager.blue_team_picks, draftManager.red_team_picks, lobby.lobby_id);
    ...
    if (roundInfo) {
        ...
        const display_time = `**${displayDateAsTimestamp(new Date(), TimestampFormat.Relative)}**`;

        embed.setColor(team === "blue" ? "#104ee0" : "#a10003");
        if (roundType === "ban") {
            embed.setDescription(
                `${team.toUpperCase()} TEAM BAN PHASE\n\n${team === "blue" ? `<@${blue_captain.user_id}>` : `<@${red_captain.user_id}>`}, it is your team's turn to ban a champion.\n${
            ...
            embed.setDescription(
                `${team.toUpperCase()} TEAM PICK PHASE\n\n${team === "blue" ? `<@${blue_captain.user_id}>` : `<@${red_captain.user_id}>`}, it is your team's turn to pick a champion.\n${
                    sendMessage ? `This round started ${display_time}` : ""
                }`,
            );
            embed.addFields({
...
async function generateComponents(blue_team, red_team) {
    ...
    const opggRow = new ActionRowBuilder().addComponents([blueTeamOPGG, redTeamOPGG]);
    return opggRow;
}
...
async function generateHeaderBar(ctx, red_captain, blue_captain) {
    ...
    ctx.fillStyle = "#104ee0";
    ctx.fillText(blueTeamText, (pickSectionWidth * 5) / 2 - blueTeamTextMetrics.width / 2, headerBarHeight / 2 + fontSize / 2);

    ctx.fillStyle = "#a10003";
    ctx.fillText(redTeamText, pickSectionWidth * 6 + (pickSectionWidth * 5) / 2 - redTeamTextMetrics.width / 2, headerBarHeight / 2 + fontSize / 2);
}

async function generateBanSection(ctx, blue_bans, red_bans, lobby_id) {
    let ban_placeholder = client.cache.get("ban_placeholder");
    if (!ban_placeholder) {
        ban_placeholder = await prepareImage(banPlaceholderPath, banSectionWidth, banSectionHeight);
    ...
    const blueBanImages = await Promise.all(
        blue_bans.map(async (champId) => {
            ...
            let square_icon = client.cache.get(`champ_${champId}_square_icon`, "lobby_id_" + lobby_id);

            if (!square_icon) {
                const champ = await Champion.findByPk(champId);
                square_icon = await loadImage(champ.square_icon);
                client.cache.set(`champ_${champId}_square_icon`, square_icon, "lobby_id_" + lobby_id);
            }
    ...
    blueBanImages.forEach((image, index) => {
        const xPosition = index * banSectionWidth + (image === ban_placeholder ? banPadding : 0);

        ctx.drawImage(image, xPosition, headerBarHeight, banSectionWidth, banSectionHeight);
    });

    const redTeamStartX = pickSectionWidth * 11 - banSectionWidth * 5;

    redBanImages.forEach((image, index) => {
        const xPosition = redTeamStartX + index * banSectionWidth + (image === ban_placeholder ? banPadding : 0);
...
async function generatePickSection(ctx, blue_picks, red_picks, lobby_id) {
    ...
    if (!pick_placeholder) {
        pick_placeholder = await prepareImage(pickPlaceholderPath, pickSectionWidth, pickSectionHeight);
    ...
    const bluePickImages = await Promise.all(
        blue_picks.map(async (champId) => {
            ...
            let loading_splash = client.cache.get(`champ_${champId}_square_icon`, "lobby_id_" + lobby_id);

            if (!loading_splash) {
                const champ = await Champion.findByPk(champId);
                loading_splash = await loadImage(champ.loading_splash);
                client.cache.set(`champ_${champId}_loading_splash`, loading_splash, "lobby_id_" + lobby_id);
            }
    ...
    bluePickImages.forEach((image, index) => {
        ...
        ctx.drawImage(image, xPosition, headerBarHeight + banSectionHeight + namePlateHeight, pickSectionWidth, pickSectionHeight);
    });
...
async function generateExtraInfo(ctx) {
    ...
    ctx.drawImage(logo, pickSectionWidth * 5, headerBarHeight, extraInfoHeight, extraInfoWidth);
}
