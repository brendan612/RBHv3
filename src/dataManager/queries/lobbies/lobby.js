const { Lobby, Match, MatchPlayer, Season, sequelize, Sequelize, User } = require("../../../models");
 ...
 * @returns {Match[]}
 */
async function getMatchHistoryForUser(user_id, game_id, season_id, page = 1, pageSize = 10) {
    try {
 ...
 * @returns {Match[]}
 */
async function getMatchHistoryForServer(game_id, season_id, page = 1, pageSize = 10) {
    try {
        return await Match.findAll({
            where: {
                end_time: { [Op.ne]: null },
