...
const level_roles = require(`../../../${process.env.CONFIG_FILE}`).roles.level_roles;
...
class UserLevelManager {
    ...
    async updateUserExperience(user_id) {
        ...
        const allowExperience = currentTimestamp - lastMessageTimestamp > this.exp_timeout;

        if (allowExperience) {
            const { level, remainingExp } = this.calculateLevelAndRemainingExp(user.server_experience);
    ...
    getRoleTitleForProfile(level, user_id = "") {
        ...
        const { suitNumber, suitName, cardNumber } = this.getSuitNumberNameAndCardNumber(level);
        ...
        switch (suitNumber) {
            ...
            case 6:
                ...
                break;
            default:
                title = `${cardNumber} of ${suitName.charAt(0).toUpperCase() + suitName.slice(1)}`;
        }
        return title;
    }
    ...
    async assignLevelRole(user_id, level, sendMessage = false) {
        ...
        const rolesToRemove = member.roles.cache.filter((r) => Object.values(level_roles).includes(r.id));
        ...
        if (sendMessage) {
            ...
            await generateLevelUpEmbed(user_id, level, this.calculateLevelReward(level), title);
        }
    }
}
