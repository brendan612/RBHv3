...
class PlayerDraftManager {
    ...
    static async getRanks(players) {
        ...
        for (const player of players) {
            ...
            if (!rank && user_id > 20) {
                try {
                    rank = await getRankByRiotID(player.summoner_name, player.tag_line, player.region_id);
        ...
        return new Map(players.map((player) => [player.user_id, PlayerDraftManager.ranks.get(player.user_id)]));
    }
