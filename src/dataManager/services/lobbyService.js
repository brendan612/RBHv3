...
const { Lobby, User, Season, Game, Draft, MatchPlayer, Match, PlayerDraftRound, DraftRound, Sequelize, Region, ServerChannel } = require("../../models/index");
...
class LobbyService {
    ...
    async destroyLobby(force = false) {
        ...
        if (this.lobby.draft_id) {
            ...
            if (draft?.thread_id) {
                try {
                    await ThreadManager.deleteThread(lobbyDTO.channels.get("general"), draft.thread_id);
                    draft.thread_id = null;
                    await draft.save();
                } catch {}
            } else if (draft?.message_id) {
                try {
                    const message = await lobbyDTO.channels.get("general").messages.fetch(draft.message_id);
                    if (message) {
            ...
            try {
                const message = await lobbyDTO.channels.get("general").messages.fetch(this.lobby.message_id);
                if (message) {
                    await message.delete();
                }
            } catch (err) {}
        ...
        for (const user of users) {
            try {
                ...
                await lobbyDTO.channels.get("general").permissionOverwrites.delete(guildMember);
                const userService = new UserService(user);
                await userService.removeRole(permission_roles.lobby_participant);
            } catch {}
        }
    ...
    async moveUsersAndDestroyLobbyVoiceChannel(postGame = false) {
        ...
        const channelToMoveTo = ChannelManager.getChannelViaServerChannel(lobby.game_id, "GLOBAL", channelToMoveToPurpose);
        ...
        const redVoice = ChannelManager.getChannelViaServerChannel(lobby.game_id, lobby.region_id, lobby.lobby_name + "_" + "Red");

        const blueVoice = ChannelManager.getChannelViaServerChannel(lobby.game_id, lobby.region_id, lobby.lobby_name + "_" + "Blue");
    ...
    async isJoinable(user_id = "") {
        ...
        if (user_id) {
            const userInLobby = lobbyUsers.find((lobbyUser) => lobbyUser.user_id === user_id);

            if (userInLobby) return { isJoinable: false, reason: "User already in lobby" };
            return {
    ...
    async isDroppable(user_id = "") {
        ...
        if (user_id) {
            const userInLobby = lobbyUsers.find((lobbyUser) => lobbyUser.user_id === user_id);
            if (!userInLobby) return { isDroppable: false, reason: "User not in lobby" };
    ...
    async handleButton(interaction, action) {
        switch (action) {
            ...
            case "drop":
                ...
                break;
            case "draft":
                const canUse = interaction.user.id == this.lobby.host_id || hasRequiredRoleOrHigher(interaction.member, "moderator");
    ...
    async draft() {
        ...
        const thread = await ThreadManager.createChannelThread(lobbyDTO.channels.get("general"), draft.draft_id);
