const { User, UserEloRating, Referral, ModerationLog, sequelize } = require("../../models/index.js");
const UserDTO = require("../DTOs/userDTO.js");
const { generateVerifyEmbed, generateProfileEmbed } = require("../messages/userEmbed.js");
const { GuildMember, ButtonBuilder, ButtonStyle, ActionRowBuilder, Interaction } = require("discord.js");
const { getSummonerByRiotID, getRankByRiotID, getRiotAccountByPuuid } = require("../../api/riot/riotApiHandler.js");
const { LeagueTier, LeagueTierHierarchy } = require("../../components/leagueRankedEnums.js");
const client = require("../../client.js");
const UserLevelManager = require("../managers/userLevelManager.js");
const roles = require(`../../../${process.env.CONFIG_FILE}`).roles.game_roles["League of Legends"];
...
const donor_roles = require(`../../../${process.env.CONFIG_FILE}`).roles.donor_roles;
...
class UserService {
    ...
    async verifyUser(user_id, game_name, tag_line, puuid, referrer_id, region_id) {
        const user = await User.findByPk(user_id);
    ...
    async syncUserRiotAccount(user_id) {
        ...
        await this.updateUserRiotAccount(user.user_id, account.gameName, account.tagLine, account.puuid);
        return new UserDTO(user);
    }
    ...
    async handleVerifyInteraction(interaction) {
        ...
        if (user_id !== member.id && !hasRequiredRoleOrHigher(member, "developer")) {
        ...
        const user = await User.findByPk(user_id);
        await getSummonerByRiotID(game_name, tag_line, user.region_id).then((summoner) => {
        ...
        await getRankByRiotID(game_name, tag_line, user.region_id).then((summoner) => {
            function isValidRank(summoner) {
                return LeagueTierHierarchy.indexOf(summoner.tier) >= LeagueTierHierarchy.indexOf(LeagueTier.GOLD);
            }
            if (!summoner) return;
            if (Array.isArray(summoner)) {
                ...
                if (summoner.length >= 1) {
                    ...
                    if (summoner.length > 1 && isValidRank(summoner[1])) {
                        highestIndex = Math.max(highestIndex, LeagueTierHierarchy.indexOf(summoner[1].tier));
                    }
                }
                validRank = highestIndex >= LeagueTierHierarchy.indexOf(LeagueTier.GOLD);
            } else {
        ...
        if (!correctIcon && !hasRequiredRoleOrHigher(member, "developer")) {
            ...
            const newVerifyButton = new ButtonBuilder().setCustomId("verified").setStyle(ButtonStyle.Success).setLabel("Verified!").setDisabled(true);
    ...
    async addMatchReward(winLoss, transaction) {
        const userLevelManager = new UserLevelManager();
        this.user.server_money += userLevelManager.calculateMatchReward(this.user.server_level, winLoss);
    ...
    async handleLeagueRoleSelectMenu(interaction, values) {
        try {
            ...
            await this.setPrimarySecondaryRoles(selectedRoles[0].name, selectedRoles[1].name);
    ...
    static async getTopReferrersWithPagination(page, pageSize) {
        ...
        const topReferrers = await User.findAll({
            ...
            attributes: ["user_id", [Sequelize.fn("COUNT", Sequelize.col("Referrals.referral_id")), "referralCount"]],
            group: ["user_id"],
    ...
    async getDonorRole(user_id) {
        ...
        const donor = member.roles.cache.find((role) => role.id === donor_roles["donor"]);
        const sponsor = member.roles.cache.find((role) => role.id === donor_roles["sponsor"]);
        const subsidizer = member.roles.cache.find((role) => role.id === donor_roles["subsidizer"]);
    ...
    async ihban(moderator_id, bannedUntil, reason) {
        const log = await ModerationLog.createModerationLog(moderator_id, this.user_id, bannedUntil, ActionType.IHBAN, reason);
        ...
        const channel = ChannelManager.getChannelViaServerChannel(1, "GLOBAL", "reports");
    ...
    async ihunban(moderator_id, reason) {
        const log = await ModerationLog.createModerationLog(moderator_id, this.user_id, null, ActionType.IHUNBAN, reason);
    }
    ...
    async verifyLobbyCommandPermission(interaction) {
        ...
        if (isBanned) {
            await interaction.reply({
                content: `You are banned from using lobby commands.\n Expires: ${displayDateAsTimestamp(remaining, TimestampFormat.Relative)}\n Reason: ${reason}`,
                ephemeral: true,
            });
            return false;
        }
