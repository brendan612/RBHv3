...
class PlayerDraftService {
    ...
    constructor(draft) {
        this.draft = draft;
        this.playerDraftManager = client.managers.playerDraftManagerFactory.getPlayerDraftManager(this.draft.draft_id);
    }
    ...
    async generatePlayerDraftEmbed(draft, sendMessage = true) {
        const { generatePlayerDraftEmbed } = require("../messages/playerDraftEmbed");
    ...
    async setSidesAndCaptains(user_id, side) {
        ...
        let canPickSides = user_id == lobby.host_id || hasRequiredRoleOrHigher(member, "admin");

        let pickingCaptain = this.playerDraftManager.picking_captain;
        let otherCaptain = this.playerDraftManager.captains.find((captain) => captain.user_id !== pickingCaptain.user_id);

        if (pickingCaptain.user_id !== user_id && !canPickSides) {
            throw new Error(`You are not the picking captain. <@${pickingCaptain.user_id}> is.`);
        }
        ...
        await this.generatePlayerDraftEmbed(new DraftDTO(await Draft.findByPk(this.draft.draft_id)), true);
    }
    ...
    async handlePlayerSelectMenu(interaction, values) {
        ...
        const canPick = allowedUsers.includes(user_id) || hasRequiredRoleOrHigher(interaction.member, "moderator");
        ...
        if (currentTeam === "red") {
            ...
            if (this.playerDraftManager.currentRound === 5) {
                ...
                const remaining = lobby.players.filter((player) => {
                    return !pickedPlayers.map((player) => player.user_id).includes(player.user_id);
                })[0];
                await this.addPlayerToTeamByUserID(remaining.user_id, "blue", this.playerDraftManager.currentRound);
                draftOver = true;
            }
        } else {
            if (canPick) {
        ...
        this.playerDraftManager.currentRound++;
        this.playerDraftManager.currentTeam = this.playerDraftManager.currentTeam === "red" ? "blue" : "red";
