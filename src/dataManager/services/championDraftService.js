const { Draft, Lobby, User, UserEloRating, DraftRound, MatchPlayer, Match, PlayerDraftRound } = require("../../models/index.js");
...
class ChampionDraftService {
    ...
    async startChampionDraft() {
        const draftManager = client.managers.draftManagerFactory.getDraftManager(this.draft.draft_id);
        ...
        try {
            const redVoice = ChannelManager.getChannelViaServerChannel(lobby.game_id, lobby.region_id, lobby.lobby_name + "_Red");

            const blueVoice = ChannelManager.getChannelViaServerChannel(lobby.game_id, lobby.region_id, lobby.lobby_name + "_Blue");
    ...
    async handleChampSelect(interaction, champion, commandRoundType, aram = false) {
        ...
        const isPickingCaptain = (team === "blue" && this.draft.blue_captain_id === user_id) || (team === "red" && this.draft.red_captain_id === user_id);

        const member = interaction.member;

        const canPick = isPickingCaptain || hasRequiredRoleOrHigher(member, "moderator");
        if (!canPick) {
        ...
        const draftOver = draftManager.currentRound == draftManager.roundSequence.length;
    ...
    async generateChampionDraftEmbed(draft, sendMessage = true) {
        const { generateChampionDraftEmbed } = require("../messages/championDraftEmbed");
        if (sendMessage) {
