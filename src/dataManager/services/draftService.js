const { Draft, Lobby, UserEloRating, MatchPlayer, Match } = require("../../models/index.js");
...
class DraftService {
    ...
    async pickCaptains(lobby) {
        ...
        if (selectedCaptains.length < 2) {
            selectedCaptains = selectedCaptains.concat(ineligiblePlayers.slice(0, 2 - selectedCaptains.length));
        }
    ...
    async startPlayerDraft() {
        ...
        const playerDraftManager = client.managers.playerDraftManagerFactory.getPlayerDraftManager(this.draft.draft_id);
        ...
        await playerDraftService.generatePlayerDraftEmbed(await DraftService.getDraft(this.draft.draft_id), true);

        const blue = await ChannelManager.createChannelForLobby(ChannelType.GuildVoice, lobby, "Blue");
        const red = await ChannelManager.createChannelForLobby(ChannelType.GuildVoice, lobby, "Red");
    }
}
