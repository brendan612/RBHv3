...
async function getSummoner(user) {
    if (user.puuid) {
        ...
        return await getSummonerByRiotID(user.summoner_name, user.tag_line, user.region_id);
    }
}
...
async function getSummonerByPuuid(puuid, region_id = "NA") {
    ...
    const summoner = await axios.get(`https://${region.platform}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${puuid}`, {
        headers: {
            "X-Riot-Token": process.env.RIOTAPIKEY,
            Origin: "https://developer.riotgames.com",
        },
...
async function getRankByRiotID(gameName, tagLine, region_id = "NA") {
    ...
    const rank = await axios.get(`https://${region.platform}.api.riotgames.com/lol/league/v4/entries/by-summoner/${summoner.id}`, {
        headers: {
            "X-Riot-Token": process.env.RIOTAPIKEY,
            Origin: "https://developer.riotgames.com",
        },
...
async function getRiotAccountByPuuid(puuid, region_id = "NA") {
    ...
    const region = await Region.findByPk(region_id);
    const riotAccount = await axios.get(`https://${region.region}.api.riotgames.com/riot/account/v1/accounts/by-puuid/${puuid}`, {
        headers: {
            "X-Riot-Token": process.env.RIOTAPIKEY,
            Origin: "https://developer.riotgames.com",
        },
