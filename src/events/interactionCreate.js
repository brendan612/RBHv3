            ...
            if (interaction.isChatInputCommand()) {
                ...
                try {
                    ...
                    switch (command.category) {
                        case "admin":
                            const allowed = await userService.verifyAdminCommandPermission(interaction);
                            if (allowed) {
                                command.execute(interaction);
                            }
                            break;
                ...
                const tournament_game_format_regex = /tournament_game_format_selection/gm;
                if (tournament_game_format_regex.test(interaction.customId)) {
                    await handleTournamentGameFormatSelectMenu(interaction, values);
                }

                const tournament_registration_type_regex = /tournament_registration_type/gm;
                ...
                const tournament_elimination_type_regex = /tournament_elimination_type/gm;
