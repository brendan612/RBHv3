...
async function handleValidThreadUser(message) {
    ...
    if (message.channel.type === ChannelType.PublicThread) {
        ...
        if (lobby) {
            ...
            if (!users.some((user) => user.user_id === user_id) && !hasRequiredRoleOrHigher(message.member, "trainee")) {
                message.delete();
            }
        }
    }
...
async function handleMessageExperience(message) {
    ...
    const allowExperience = currentTimestamp - lastMessageTimestamp > EXPERIENCE_TIMEOUT;
...
async function handleAutoResponse(message) {
    const autoResponse = await AutoResponse.findOne({
        where: Sequelize.where(Sequelize.fn("LOWER", Sequelize.col("trigger")), Sequelize.fn("LOWER", message.content)),
    });
