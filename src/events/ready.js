// @ts-nocheck
const { Events, ActivityType, Guild } = require("discord.js");
const webhookServer = require("../handlers/voteHandler");
const { checkAndUpdateServerMessages } = require("../handlers/serverMessageHandler");
const { channels } = require(`../../${process.env.CONFIG_FILE}`);
const { ServerMessage, Match, Lobby, Draft, MatchPlayer, User, Season, Game, UserEloRating, Referral, FeatureToggle, AutoResponse, ServerChannel, ServerRole, sequelize } = require("../models");
...
module.exports = {
    ...
    async execute(client) {
        ...
        const serverRoles = await ServerRole.findAll();
        client.serverRoles = serverRoles;
    },
};
...
const createLobbies = async (db) => {
    return new Promise((resolve, reject) => {
        db.all("select * from lobbies where seasonId in (select pid from seasons where gameId = 1)", [], async (err, rows) => {
            if (err) {
                throw err;
            }
            try {
            ...
            await Promise.all(
                matchChunk.map(async (match) => {
                    const rows2 = await new Promise((resolve, reject) => {
                        db.all(`select * from lobby_players where lobbyId = '${match.match_id}'`, [], (err, rows) => {
                            if (err) {
                                reject(err);
                            }
                            resolve(rows);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            const rows = await new Promise((resolve, reject) => {
                db.all("SELECT CAST(referrerPID AS TEXT) as referrer_id, CAST(userPID AS TEXT) as user_id FROM referrals", [], (err, rows) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(rows);
...
const translateDataToMatch = (sqliteRow) => {
    let seasonIndex = old_seasons.findIndex((season) => season.season_game_id === sqliteRow.seasonId);
...
const registerFonts = () => {
    GlobalFonts.registerFromPath(path.join(__dirname, "../assets/fonts/postgame.ttf"), "PostGame");
    GlobalFonts.registerFromPath(path.join(__dirname, "../assets/fonts/NotoSerifKR-Regular.otf"), "Noto Serif KR");
};
